function [myH,varargout]=show_cm_extended(CM,CC,CC2,cod,mode,varargin)
...
if mode==1,
    ...
    elseif mode==3
        ...
        % plot graph
        [myH{H_S},myH{H_T},wCM]=brain_plot3d(CM,CC,CC2,cod,pp,CMold);
        varargout(1)={wCM};
...
function myH=brain_plot(CM,CC,cod,coor,pp) % 2D
...
function [myHs,myHt,wCM]=brain_plot3d(CM,CC,CC2,cod,pp,CMold)
% IN:
...
%         end
        ...
        if foo_plotDS==true
            ...
            if pp.SphBlack==true && ~(abs(pp.FctForColSph(iter))>=pp.plotDegreeSpheresOnlyAboveThreshold)
                ...
                %myCoData=repmat([1 1 1],size(sZ));
                
            end
            
            caxis([-max([abs(tmp1),tmp2]),max([abs(tmp1),tmp2])]);
            myHs(end+1)=surf(sX,sY,sZ,'FaceAlpha',1.0,'edgecolor','none','CData',myCoData);
