...
rep = 200;     % nullset dimension

%F_TASK = F_RECOMPUTE;
F_TASK = F_RELOAD;

filename = 'fMRI_singlePatient';


if(F_TASK == F_RECOMPUTE)  
   for npat = 1:21
      data = zX_RS(:,:,npat:npat);
      [n, T, ~] = size(data);
      disp(['patient = ', num2str(npat)])
...
xticks(1:npat)
xticklabels(lbs)
xlabel('patient considered'); ylabel('k4_{bw-fw}'); 
legend(L, {'non-significant', 'significant'}, 'Location', 'NorthEast');
